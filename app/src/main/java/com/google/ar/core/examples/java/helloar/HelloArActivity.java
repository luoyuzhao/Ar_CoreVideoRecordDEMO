 ...
 */
public class HelloArActivity extends AppCompatActivity implements GLSurfaceView.Renderer,VideoRecorder.VideoRecorderListener {
  ...
  private void initAndroidEGLConfig(EGLConfig config){
  ...
  public int DEFAULT_BITRATE=4000000;
  public void clickToggleRecording(View view) {
    Log.d(TAG, "clickToggleRecording");
    if (mRecorder == null) {
      File outputFile = new File(Environment.getExternalStorageDirectory() + "/HelloAR",
              "HelloAR-" + Long.toHexString(System.currentTimeMillis()) + ".mp4");
      File dir = outputFile.getParentFile();
      if (!dir.exists()) {
        dir.mkdirs();
      }
      try {
        //test Size 640*480,真实分辨率请从摄像头读取
        mRecorder = new VideoRecorder(640,
                480,
                DEFAULT_BITRATE, outputFile, this);
      ...
      if (mRecorder!= null && mRecorder.isRecording()) {
        ...
        if (ctx != null) {
          ...
          draw(frame, camera.getTrackingState() == TrackingState.PAUSED,
                  viewmtx, projmtx, camera.getDisplayOrientedPose());
          // restore the context
  ...
  private void draw(Frame frame, boolean paused,
    ...
    for (ColoredAnchor anchor : anchors) {
      if (anchor.anchor.getTrackingState() != TrackingState.TRACKING) {
        continue;
      }
      ...
      anchor.anchor.getPose().toMatrix(anchorMatrix, 0);
      // Update and draw the model and its shadow.
      virtualObject.updateModelMatrix(anchorMatrix, scaleFactor);
      virtualObjectShadow.updateModelMatrix(anchorMatrix, scaleFactor);
      virtualObject.draw(viewMatrix, projectionMatrix, anchor.color);
      virtualObjectShadow.draw(viewMatrix, projectionMatrix, anchor.color);
    }
  }
